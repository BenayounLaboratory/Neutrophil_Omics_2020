...
source('Parsing_gmt_features_functions.R')

###################################################################################################
####### A. Gene Expression feature (response curve)
###################################################################################################

my.expressed.genes <- read.csv('/Volumes/BB_Home/Neutrophils/Neutrophil_analysis/RNA-seq/DEseq2/STAR/2020-04-02_Neutrophils_DESeq2_Analysis_STAR_post_SVA_SEX_DIM_all_genes_statistics.txt', sep = "\t", header = T)
...
for (i in 1:nrow(my.RNA.features)) {
  ...
  if (my.RNA.features$FDR[i] < 0.05) {
    ...
    if (my.RNA.features$logFC[i] > 0) {
      ...
      my.RNA.features$SEX_DIMORPHISM[i] <- "FEMALE"
      
    }  else if (my.RNA.features$logFC[i] < 0) {
      
      my.RNA.features$SEX_DIMORPHISM[i] <- "MALE"
      
    }
  }
}

my.RNA.features  <- data.frame(my.RNA.features[my.RNA.features$SEX_DIMORPHISM != "NON_SEX_DIM",])
my.RNA.features$SEX_DIMORPHISM <- factor(my.RNA.features$SEX_DIMORPHISM)

summary(my.RNA.features$SEX_DIMORPHISM)
# FEMALE   MALE 
# 1073    661 

save(my.RNA.features, 
     file = paste(Sys.Date(),"RNA_features_for_ML.RData", sep = "_"))
...
my.coll.GEO             <- read.gmt("../TF_GMT_cleanup/2020-05-20_GEO_Harmonizome_Target_summary.gmt")
...
dim(my.TF.features.ALL) # 1734  342
...
save(my.TF.features.FINAL, file = paste(Sys.Date(),"summary_TF_features_from_GMT_for_ML.RData", sep = "_"))
...
my.homer.prom.data.1 <- read.csv('../Constant_features/2020-04-28_HOMER_Neutrophil_Promoter_features.txt', sep = "\t", header = T)
...
# [1] "chr16"                "chr5"                 "chr17"                "chr2"                 "chr10"               
# [6] "chr13"                "chr15"                "chr9"                 "chr4"                 "chr3"                
# [11] "chr11"                "chr7"                 "chr1"                 "chr6"                 "chr18"               
# [16] "chr8"                 "chrX"                 "chr12"                "chr14"                "chr19"               
# [21] "chrY"                 "chr1_GL456221_random" "chr4_GL456216_random" "chrUn_JH584304"       "chr1_GL456211_random"
# [26] "chrX_GL456233_random"
...
my.homer.prom.data.2$SexChr <- 0   # start with autosomal
my.homer.prom.data.2$SexChr[my.homer.prom.data.2$Chrosomome %in% "chrX"] <- 1  
my.homer.prom.data.2$SexChr[my.homer.prom.data.2$Chrosomome %in% "chrY"] <- 1  

####################################
# Homer gene features
my.homer.gene.data.1 <- read.csv('../Constant_features/2020-04-28_HOMER_Neutrophil_Gene_features.txt', sep = "\t", header = T)
...
save(my.seq.feat.alls, 
     file = paste(Sys.Date(),"HOMER_sequence_features_for_ML.RData", sep = "_"))

dim(my.seq.feat.alls)
###[1] 1636    7
# only 1636 genes could have features extracted

###################################################################################################
####### D. ATAC-seq features
###################################################################################################

my.ATAC.seq.data <- read.csv('../ATAC/HOMER_Neutrophil_ATAC_seq_reads.txt', sep = "\t", header = T)
...
my.ATAC.seq.data.3  <- data.frame("Gene.Name"                =  my.ATAC.seq.data.2$Gene.Name,
                                  ...
                                  "log2_ATAC_TSS_FPKM_F_over_M"   =  log2( (apply(my.ATAC.seq.data.2[,c(2,4)],1,mean)/ (apply(my.ATAC.seq.data.2[,c(3,5)],1,mean) + 0.01)) + 0.01 )
                                  )
...
save(my.ATAC.seq.features, 
     file = paste(Sys.Date(),"ATACseq_features_for_ML.RData", sep = "_"))


#######################
sink(file = paste(Sys.Date(),"Machine_Learning_Feature_engineering_R_session_Info.txt", sep =""))
sessionInfo()
sink()
