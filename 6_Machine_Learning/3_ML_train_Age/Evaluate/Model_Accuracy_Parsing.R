setwd('/Volumes/BB_Home/Neutrophils/Neutrophil_analysis/Machine_Learning/ML_Predict_Age_Expression/ML_run/Testing_accuracy/')
options(stringsAsFactors = F)

source('accuracy_parsing_functions_AGING.R')

# Run same framework as sex-dimorphism (no Sex Chr, since it does not make sense for aging)
...
load('../Aging_ML/2020-06-01_Training_Data_AGING.RData')
...
########################################################################################
# Load models
load("../Aging_ML/2020-06-01_LDA_model_AGING.RData")
load("../Aging_ML/2020-06-01_NNET_model_AGING.RData")
load("../Aging_ML/2020-06-01_RF_model_AGING.RData")
load("../Aging_ML/2020-06-01_SVM_model_AGING.RData")
load("../Aging_ML/2020-06-02_GBM_model_AGING.RData")
load("../Aging_ML/2020-06-01_cTree_model_AGING.RData")
load("../Aging_ML/2020-06-01_Regularized_Logistic_Regression_model_AGING.RData")

Acc.svm.fit     <- get_acc_metrics("Neutrophil_Aging_SVM"                  , my.svm.fit      , my.testing.data)
Acc.lda.fit     <- get_acc_metrics("Neutrophil_Aging_LDA"                  , my.lda.fit      , my.testing.data)
Acc.nnet.fit    <- get_acc_metrics("Neutrophil_Aging_NNET"                 , my.nnet.fit     , my.testing.data)
Acc.rf.fit      <- get_acc_metrics("Neutrophil_Aging_RF"                   , my.rf.fit       , my.testing.data)
Acc.gbm.fit     <- get_acc_metrics("Neutrophil_Aging_GBM"                  , my.gbm.fit      , my.testing.data)
Acc.ctree.fit   <- get_acc_metrics("Neutrophil_Aging_cTree"                , my.ctree.fit    , my.testing.data)
Acc.logreg.fit  <- get_acc_metrics("Neutrophil_Aging_Regularized_Logistic"  , my.logreg.fit   , my.testing.data)
...
pdf(paste0(Sys.Date(),"_Testing_Balanced_accuracy_dotplot_AGING.pdf"))
...
pdf(paste0(Sys.Date(),"_Testing_AUC_dotplot_AGING.pdf"))
...
pdf(paste0(Sys.Date(),"_Testing_Accuracy_AUC_heatmap_AGING.pdf"))
...
save(my.var.Imps, file = paste0(Sys.Date(),"_Variable_Importance_Parsing_RF_GBM_CARET_AGING.RData"))
write.table(my.var.Imps,file = paste0(Sys.Date(),"_Variable_Importance_Parsing_RF_GBM_CARET_AGING.txt"), quote = F, row.names = F, sep = "\t")

# feature importance on tree based models
my.rf.varimps.native <- my.rf.fit$finalModel$importance[,3:4]
save(my.rf.varimps.native, file = paste0(Sys.Date(),"_Variable_Importance_RF_native_AGING.RData"))
########################################################################################
...
my.pdfname <-paste(Sys.Date(),"RF_GBM_Models_VarImps_balloon_plot_top_20_AGING.pdf", sep="_")
