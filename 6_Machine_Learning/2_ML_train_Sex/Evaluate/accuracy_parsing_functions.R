...
## DEBUG
# my.model.name <- "Neutrophil_Sex_Dim_SVM"
...
get_acc_metrics <- function(my.model.name, my.mod.fit, my.testing) {
  ...
  my.confus.mat <- confusionMatrix(my.mod.preds,my.testing$SEX_DIMORPHISM) # prediction, then ref
  ...
  my.filename <- paste(Sys.Date(),my.model.name,"ROC_testing_classification.pdf",sep="_")
  ...
  write.table(data.frame(balacc      = my.bal.acc, 
                         ...
                         precision   = my.prec), 
              row.names = F,
              file= paste(Sys.Date(),my.model.name,"testing_metrics_classification.txt", sep="_"))
...
get_roc <- function(my.mod.fit,my.testing) {
  ...
  my.roc <- roc(my.testing$SEX_DIMORPHISM, my.testing.preds.probs$MALE) # Calculate ROC curve.
