...
# summary(my.ALL.features.2group[,1:20])
...
# get data partition
set.seed(123456789)
my.training.idx <- createDataPartition(my.ALL.features.2group.inc$SEX_DIMORPHISM, p=0.67, list=FALSE) # 2/3 for training

my.training.data <- my.ALL.features.2group.inc[my.training.idx,]

save(my.training.data, my.training.idx, file = paste0(Sys.Date(),"_Training_Data.RData"))
...
my.svm.fit       <- train( SEX_DIMORPHISM ~ .,
...
save(my.svm.fit, file = paste0(Sys.Date(),"_SVM_model.RData"))
...
# train model with caret train function
my.lda.fit       <- train( SEX_DIMORPHISM ~ .,
                           data = my.training.data,
                           method="sparseLDA",
...
save(my.lda.fit, file = paste0(Sys.Date(),"_LDA_model.RData"))
...
# train model with caret train function
my.nnet.fit <- train( SEX_DIMORPHISM ~ .,
                      data = my.training.data,
                      method="pcaNNet",
...
save(my.nnet.fit, file = paste0(Sys.Date(),"_NNET_model.RData"))
...
# train model with caret train function
my.logreg.fit <- train( SEX_DIMORPHISM ~ .,
                        data = my.training.data,
                        method="regLogistic",
...
save(my.logreg.fit, file = paste0(Sys.Date(),"_Regularized_Logistic_Regression_model.RData"))
...
# train model with caret train function
my.rf.fit <- train( SEX_DIMORPHISM ~ .,
                    data = my.training.data,
                    method="rf",
                    importance=TRUE,
...
my.ctree.fit       <- train( SEX_DIMORPHISM ~ .,
                             data = my.training.data,
                             method="ctree",
...
save(my.ctree.fit, file = paste0(Sys.Date(),"_cTree_model.RData"))
...
# train model with caret train function
my.gbm.fit <- train( SEX_DIMORPHISM ~ .,
                     data = my.training.data,
                     method="gbm",
...
save(my.gbm.fit, file = paste0(Sys.Date(),"_GBM_model.RData"))

#######################
sink(file = paste(Sys.Date(),"Machine_Learning_R_session_Info.txt", sep =""))
sessionInfo()
sink()
