...
sample <- read.csv("PQI_20181010_Mouse_neutrophils_aging_HILIC_new_pos.csv")
...
length(which(rowSums(sample4[13:28] == 1) >= 5))
sample.mval <- sample4[-which(rowSums(sample4[13:28] == 1) >= 5),]

rm(list = c("sample4","sample5","sample6"))
...
write.csv(sample_dln_filt,file="BB_Mouse_neutrophils_HILIC_pos.csv")
